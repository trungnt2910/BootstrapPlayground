...
static constexpr auto MakeWdfFunctionTable(std::index_sequence<Indices...>) {
...
static constexpr auto sWdfFunctionTableStub = MakeWdfFunctionTable(
    std::make_index_sequence<kWdfFunctionTableStubCount>{});
...
static NTSTATUS NTAPI impl_WdfVersionBind(
  ...
  if (bindInfo->FuncTable != nullptr) {
    *bindInfo->FuncTable = (WDFFUNC)(PVOID)sWdfFunctionTableStub.data();
  }
...
impl_WdfVersionBindClass(PVOID context, PWDF_BIND_INFO bindInfo,
...
static VOID NTAPI
impl_WdfVersionUnbindClass(PVOID /*context*/, PWDF_BIND_INFO bindInfo,
                           PWDF_COMPONENT_GLOBALS componentGlobals) {
  NT_STUB_REPORT();
  std::flush(std::cerr);
