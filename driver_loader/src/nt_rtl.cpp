...
impl_RtlUTF8ToUnicodeN(WCHAR *dest, ULONG destLen, ULONG *resultLen, const char *src, ULONG srcLen)
{
    ...
    int n = MultiByteToWideChar(
        ...
        dest,
        dest ? static_cast<int>(destLen / static_cast<ULONG>(sizeof(WCHAR))) : 0);
    if (n == 0 && srcLen > 0)
...
static NTSTATUS NTAPI impl_RtlSetDaclSecurityDescriptor(
    PVOID /*sd*/, BOOLEAN /*present*/, PVOID /*dacl*/, BOOLEAN /*defaulted*/)
{
    NT_STUB_REPORT();
    std::flush(std::cerr);
    return STATUS_SUCCESS;
...
static NTSTATUS NTAPI
impl_RtlAddAccessAllowedAce(PVOID /*acl*/, ULONG /*aceRev*/, ULONG /*access*/, PVOID /*sid*/)
{
    NT_STUB_REPORT();
    std::flush(std::cerr);
    return STATUS_SUCCESS;
...
{
    std::println(
        stderr, "[nt_stubs] call RtlFailFast(code=0x{:X})", static_cast<unsigned long long>(code));
    std::flush(std::cerr);
    std::abort();
}
