...
std::array<std::string, 256> NtStubNameTable = {};
int NtStubNextIndex = 0;
...
[[noreturn]] void NtStubCall(int idx) noexcept
{
    const std::string& name =
        (idx >= 0 && idx < 256 && !NtStubNameTable[static_cast<std::size_t>(idx)].empty())
            ? NtStubNameTable[static_cast<std::size_t>(idx)]
            : "<unknown>";
...
void *NtStubsAllocate(const char *name) noexcept
{
    ...
    NtStubNameTable[static_cast<std::size_t>(NtStubNextIndex)] = name;
    void *ptr = reinterpret_cast<void *>(NtStubGetTable()[NtStubNextIndex]);
    ++NtStubNextIndex;
    return ptr;
}
...
static BOOLEAN NTAPI impl_RtlEqualUnicodeString(
    const UNICODE_STRING *s1, const UNICODE_STRING *s2, BOOLEAN caseInsensitive);
static VOID NTAPI impl_RtlCopyUnicodeString(UNICODE_STRING *dest, const UNICODE_STRING *src);
static LONG NTAPI impl_RtlCompareUnicodeString(
    const UNICODE_STRING *s1, const UNICODE_STRING *s2, BOOLEAN caseInsensitive);
...
impl_RtlUTF8ToUnicodeN(WCHAR *dest, ULONG destLen, ULONG *resultLen, const char *src, ULONG srcLen);
...
static NTSTATUS NTAPI
impl_RtlGetDaclSecurityDescriptor(PVOID sd, BOOLEAN *present, PVOID *dacl, BOOLEAN *defaulted);
static NTSTATUS NTAPI
impl_RtlGetGroupSecurityDescriptor(PVOID sd, PVOID *group, BOOLEAN *defaulted);
static NTSTATUS NTAPI
impl_RtlGetOwnerSecurityDescriptor(PVOID sd, PVOID *owner, BOOLEAN *defaulted);
static NTSTATUS NTAPI
impl_RtlGetSaclSecurityDescriptor(PVOID sd, BOOLEAN *present, PVOID *sacl, BOOLEAN *defaulted);
static NTSTATUS NTAPI
impl_RtlSetDaclSecurityDescriptor(PVOID sd, BOOLEAN present, PVOID dacl, BOOLEAN defaulted);
static NTSTATUS NTAPI
impl_RtlAbsoluteToSelfRelativeSD(PVOID absoluteSD, PVOID selfRelSD, ULONG *bufLen);
static NTSTATUS NTAPI
impl_RtlAddAccessAllowedAce(PVOID acl, ULONG aceRev, ULONG accessMask, PVOID sid);
static ULONG NTAPI impl_RtlLengthSid(PVOID sid);
static NTSTATUS NTAPI impl_SeCaptureSecurityDescriptor(
    PVOID srcSD, ULONG accessMode, ULONG poolType, BOOLEAN captureIfKernel, PVOID *capturedSD);
...
static PMDL NTAPI
impl_IoAllocateMdl(PVOID va, ULONG byteCount, BOOLEAN secondary, BOOLEAN chargeQuota, PIRP irp);
...
static PVOID NTAPI impl_MmMapLockedPagesSpecifyCache(
    PMDL mdl, UCHAR accessMode, ULONG cacheType, PVOID baseAddr, ULONG zeroBits, ULONG priority);
...
static NTSTATUS NTAPI impl_IoCreateSymbolicLink(UNICODE_STRING *symLink, UNICODE_STRING *devName);
...
static NTSTATUS NTAPI impl_ZwQueryValueKey(
    HANDLE key, PVOID name, ULONG keyClass, PVOID info, ULONG infoLen, ULONG *resultLen);
static NTSTATUS NTAPI
impl_ZwSetValueKey(HANDLE key, PVOID name, ULONG titleIdx, ULONG type, PVOID data, ULONG len);
static NTSTATUS NTAPI impl_ZwSetSecurityObject(HANDLE handle, ULONG secInfo, PVOID sd);
static NTSTATUS NTAPI
impl_ZwQuerySystemInformation(ULONG infoClass, PVOID info, ULONG infoLen, ULONG *returnLen);
...
// ---- Timing ----------------------------------------------------------------
static NTSTATUS NTAPI impl_KeDelayExecutionThread(ULONG mode, BOOLEAN alertable, PVOID interval);

// ---- Interlock -------------------------------------------------------------
static LONG NTAPI
impl_InterlockedCompareExchange(volatile LONG *dest, LONG exchange, LONG comparand);

// ---- WDF (wdfldr.sys) ------------------------------------------------------
static NTSTATUS NTAPI impl_WdfVersionBind(
    PDRIVER_OBJECT driverObject,
    PUNICODE_STRING registryPath,
    PWDF_BIND_INFO bindInfo,
    PWDF_COMPONENT_GLOBALS *componentGlobals);
static NTSTATUS NTAPI impl_WdfVersionBindClass(
    PVOID context, PWDF_BIND_INFO bindInfo, PWDF_COMPONENT_GLOBALS *componentGlobals);
static VOID NTAPI impl_WdfVersionUnbind(
    PUNICODE_STRING registryPath, PWDF_BIND_INFO bindInfo, PWDF_COMPONENT_GLOBALS componentGlobals);
static VOID NTAPI impl_WdfVersionUnbindClass(
    PVOID context, PWDF_BIND_INFO bindInfo, PWDF_COMPONENT_GLOBALS componentGlobals);
...
static const NtSymbol s_ntSymbols[] = {
    ...
    NT_SYM(PsProcessType),
    NT_SYM(PsThreadType),
    NT_SYM(IoDeviceObjectType),
    NT_SYM(PsInitialSystemProcess),
    NT_SYM(SeExports),
...
// #endif
                ...
                if (m)
                {
                    ...
                    if (p)
                    {
                        DL_LOG_TRACE("[nt_stubs] lookup {} -> ntdll @ {:p}", name, p);
                        return p;
                    }
                }
                ...
                if (m)
                {
                    ...
                    if (p)
                    {
                        DL_LOG_TRACE("[nt_stubs] lookup {} -> msvcrt @ {:p}", name, p);
                        return p;
                    }
                }
                ...
                const std::string msg = std::format(
                    "[nt_stubs] Warning: {} not found in ntdll/msvcrt; using fallback "
                    "implementation.\n",
                    sn);
    ...
    for (const auto &sym : s_ntSymbols)
    {
        ...
        {
            DL_LOG_TRACE("[nt_stubs] lookup {} -> builtin @ {:p}", name, sym.address);
            return sym.address;
        }
    }
    DL_LOG_TRACE("[nt_stubs] lookup {} -> <unresolved>", name);
    return nullptr;
}
